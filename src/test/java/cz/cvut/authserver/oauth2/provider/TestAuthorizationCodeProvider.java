...
import cz.cvut.authserver.oauth2.provider.ServerRunning.UriBuilder;
...
public class TestAuthorizationCodeProvider {
	...
	public void loginAndExtractCookie() {
		...
		formData.add("j_username", "tomy");
		formData.add("j_password", "best");

		String location = "/auth/login.do";
	...
	public void testResourceIsProtected() throws Exception {
		...
		assertEquals(HttpStatus.UNAUTHORIZED, serverRunning.getStatusCode("/auth/some-protected-resource?format=json"));
	}
	...
	public void testUnauthenticatedAuthorizationRequestRedirectsToLogin() throws Exception {
		...
		assertEquals(serverRunning.getUrl("/auth/login.jsp"), location);
	...
	public void testSuccessfulAuthorizationCodeFlow() throws Exception {
		...
		assertEquals(HttpStatus.OK, serverRunning.getStatusCode("/auth/some-protected-resource?format=json"));
	...
	public void testInvalidScopeInAuthorizationRequest() throws Exception {
		...
		UriBuilder uri = serverRunning.buildUri("/auth/oauth/authorize").queryParam("response_type", "code")
	...
	public void testInsufficientScopeInResourceRequest() throws Exception {
		...
		assertNotNull(context.getAccessToken());
		ResponseEntity<String> response = serverRunning.getForString("/auth/some-protected-resource?format=json");
	...
	public void testInvalidAccessToken() throws Exception {
		...
		ResponseEntity<String> response = serverRunning.getForString("/auth/some-protected-resource?format=json", headers);
		assertEquals(HttpStatus.UNAUTHORIZED, response.getStatusCode());
	...
	private String loginAndGrabCookie() {
		...
		ResponseEntity<Void> result = serverRunning.postForStatus("/auth/login.do", headers, formData);
		assertEquals(HttpStatus.FOUND, result.getStatusCode());
	...
	static class MyLessTrustedClient extends AuthorizationCodeResourceDetails {
		public MyLessTrustedClient(Object target) {
			...
			setAccessTokenUri(test.serverRunning.getUrl("/auth/oauth/token"));
			setUserAuthorizationUri(test.serverRunning.getUrl("/auth/oauth/authorize"));
		}
	}
